...
def create_and_save_checkpoint_callback(checkpoint_dir,tensorboard_log_dir):
    ...
    ckpt_callback_filepath = os.path.join(checkpoint_dir, "checkpoint_cb.cb")
...
def get_callbacks(callback_dir_path):
    callback_path=[ 
        os.path.join(callback_dir_path, bin_file) for bin_file in os.listdir(callback_dir_path) 
